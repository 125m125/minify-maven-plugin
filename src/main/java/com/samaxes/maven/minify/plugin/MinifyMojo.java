...
public class MinifyMojo extends AbstractMojo {
  ...
  /**
   * Do not append a suffix to the minified output file name, independently of the value in the
   * {@code suffix} parameter.<br/>
   * <strong>Warning:</strong> when both the options {@code nosuffix} and {@code skipMerge} are set
   * to {@code true}, the plugin execution phase needs to be set to {@code package}, otherwise the
   * output files will be overridden by the source files during the packaging.
   *
   * @since 1.7
   */
  ...
  /**
   * CSS files to include. Specified as fileset patterns which are relative to the CSS source
   * directory.
   *
   * @since 1.2
   */
  ...
  /**
   * CSS files to exclude. Specified as fileset patterns which are relative to the CSS source
   * directory.
   *
   * @since 1.2
   */
   ...
   * <ul>
   * <li>{@code YUI}: <a href="http://yui.github.io/yuicompressor/">YUI Compressor</a></li>
   * <li>{@code CLOSURE}: <a href="https://developers.google.com/closure/compiler/">Google Closure
   * Compiler</a></li>
   * </ul>
   *
   * @since 1.6
  ...
  /**
   * Some source control tools don't like files containing lines longer than, say 8000 characters.
   * The line-break option is used in that case to split long lines after a specific column. It can
   * also be used to make the code more readable and easier to debug. Specify {@code 0} to get a
   * line break after each semi-colon in JavaScript, and after each rule in CSS. Specify {@code -1}
   * to disallow line breaks.
   */
  ...
  /**
   * Preserve unnecessary semicolons (such as right before a '}'). This option is useful when
   * compressed code has to be run through JSLint.
   */
   ...
   * <li>{@code ECMASCRIPT6}: Checks code assuming ECMAScript 6 compliance, allowing new features
   * not present in ECMAScript 5.</li>
   * <li>{@code ECMASCRIPT6_STRICT}: Like {@code ECMASCRIPT6} but assumes compliance with strict
   * mode ({@code 'use strict';}).</li>
   * </ul>
   *
   * @since 1.7.2
   ...
   * It accepts the same options as {@code closureLanguageIn} and is used to transpile between
   * different levels of ECMAScript.
   *
   * @since 1.7.5
   */
  @Parameter(property = "closureLanguageOut", defaultValue = "ECMASCRIPT_2021")
   ...
   * Please visit <a href=
   * "https://docs.google.com/document/d/1U1RGAehQwRypUTovF1KRlpiOFze0b-_2gc6fAH0KY0k/edit">Source
   ...
   * If {@code true}, automatically sort dependencies so that a file that {@code goog.provides}
   * symbol X will always come before a file that {@code goog.requires} symbol X.
   *
   * @since 1.7.4
   */
   ...
   * </pre>
   * 
   * where {@code <name>} is the name of a {@code @define} variable and {@code value} is a boolean,
   * number or string.
   *
   * @since 1.7.5
   */
  ...
  @Parameter(property = "htmlReferenceReplacements", defaultValue = "true")
  private boolean htmlReferenceReplacements;

  private Map<String, String> replacementNames = new ConcurrentHashMap<>();
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    try {
      new FileReferenceReplacer(replacementNames, webappTargetDir, verbose, getLog(), jsSourceDir,
          cssSourceDir, htmlSourceDir, charset).process();
    } catch (IOException e) {
      throw new MojoExecutionException(e.getMessage(), e);
    }
  }
  ...
  private ClosureConfig fillClosureConfig() throws MojoFailureException {
    ...
    for (String extern : closureExterns) {
      externs.add(
          SourceFile.fromFile(webappSourceDir + File.separator + extern, Charset.forName(charset)));
    }
    ...
    for (Map.Entry<String, String> warningLevel : closureWarningLevels.entrySet()) {
      ...
      if (diagnosticGroup == null) {
        throw new MojoFailureException("Failed to process closureWarningLevels: "
            + warningLevel.getKey() + " is an invalid DiagnosticGroup");
      }
      ...
      try {
        ...
        throw new MojoFailureException("Failed to process closureWarningLevels: "
            + warningLevel.getKey() + " is an invalid CheckLevel");
      }
    }

    return new ClosureConfig(closureLanguageIn, closureLanguageOut, closureEnvironment,
        closureCompilationLevel, dependencyOptions, externs, closureCreateSourceMap, warningLevels,
        closureAngularPass, closureExtraAnnotations, closureDefine);
  }
  ...
  private Collection<ProcessFilesTask> createTasks(YuiConfig yuiConfig, ClosureConfig closureConfig)
    ...
    if (!Strings.isNullOrEmpty(bundleConfiguration)) { // If a bundleConfiguration is defined,
                                                       // attempt to use that
      ...
      tasks.add(createCSSTask(yuiConfig, closureConfig, cssSourceFiles, cssSourceIncludes,
          cssSourceExcludes, cssFinalFile));
      tasks.add(createJSTask(yuiConfig, closureConfig, jsSourceFiles, jsSourceIncludes,
          jsSourceExcludes, jsFinalFile));
      tasks.add(createHTMLTask(yuiConfig, closureConfig, htmlSourceFiles, htmlSourceIncludes,
          htmlSourceExcludes, htmlFinalFile));
    }
  ...
  private ProcessFilesTask createCSSTask(YuiConfig yuiConfig, ClosureConfig closureConfig,
    ...
    return new ProcessCSSFilesTask(getLog(), verbose, bufferSize, Charset.forName(charset), suffix,
        nosuffix, skipMerge, skipMinify, webappSourceDir, webappTargetDir, cssSourceDir,
        cssSourceFiles, cssSourceIncludes, cssSourceExcludes, cssTargetDir, cssFinalFile, cssEngine,
        yuiConfig, replacementNames);
  }

  private ProcessFilesTask createJSTask(YuiConfig yuiConfig, ClosureConfig closureConfig,
      List<String> jsSourceFiles, List<String> jsSourceIncludes, List<String> jsSourceExcludes,
      String jsFinalFile) throws FileNotFoundException {
    return new ProcessJSFilesTask(getLog(), verbose, bufferSize, Charset.forName(charset), suffix,
        nosuffix, skipMerge, skipMinify, webappSourceDir, webappTargetDir, jsSourceDir,
        jsSourceFiles, jsSourceIncludes, jsSourceExcludes, jsTargetDir, jsFinalFile, jsEngine,
        yuiConfig, closureConfig, replacementNames);
  }

  private ProcessFilesTask createHTMLTask(YuiConfig yuiConfig, ClosureConfig closureConfig,
      List<String> htmlSourceFiles, List<String> htmlSourceIncludes,
      List<String> htmlSourceExcludes, String htmlFinalFile) throws FileNotFoundException {
    return new ProcessHTMLFilesTask(getLog(), verbose, bufferSize, Charset.forName(charset), suffix,
        nosuffix, skipMerge, skipMinify, webappSourceDir, webappTargetDir, htmlSourceDir,
        htmlSourceFiles, htmlSourceIncludes, htmlSourceExcludes, htmlTargetDir, htmlFinalFile,
        cssEngine, yuiConfig, replacementNames);
  }
}
